...
func TestCreateRental(t *testing.T) {
	...
	tests := []struct {
		name                string
		inputRental         rentals_service.Rentals
	...
	}{
		{
			...
			inputRental: rentals_service.Rentals{EquipmentId: 1},
			mockRentals: func(m *mock_rentals_service.MockRentalsRepo) {},
			mockEquipments: func(m *mock_equipments_service.MockEquipmentsRepo) {
				m.EXPECT().GetById(1).Return(equipments_service.Equipments{}, errors.New("record not found"))
			},
			mockUsers:           func(m *mock_users_service.MockUsersRepo) {},
			mockRentalHistories: func(m *mock_rental_histories_service.MockRentalHistoriesRepo) {},
			mockXendit:          func(m *mock_xendit_service.MockXenditRepo) {},
		...
		},
		{
			...
			mockRentals: func(m *mock_rentals_service.MockRentalsRepo) {},
			mockEquipments: func(m *mock_equipments_service.MockEquipmentsRepo) {
				m.EXPECT().GetById(1).Return(equipments_service.Equipments{
					Id:            1,
					Name:          "Farhan",
					CategoryId:    2,
					Description:   "farhan",
			...
			mockUsers: func(m *mock_users_service.MockUsersRepo) {
				m.EXPECT().FindById("410dkjfboi").Return(users_service.UsersResponse{}, errors.New("record not found"))
			},
			mockRentalHistories: func(m *mock_rental_histories_service.MockRentalHistoriesRepo) {},
			mockXendit:          func(m *mock_xendit_service.MockXenditRepo) {},
			mockPayments:        func(m *mock_payments_service.MockPaymentsRepo) {},
		...
		},
		{
			...
			inputRental: rentals_service.Rentals{
				...
				UserId:      "410dkjfboi"},
			mockRentals: func(m *mock_rentals_service.MockRentalsRepo) {
				m.EXPECT().Create(gomock.Any()).Return(rentals_service.Rentals{}, errors.New("There is no data"))
			},
			mockEquipments: func(m *mock_equipments_service.MockEquipmentsRepo) {
				m.EXPECT().GetById(1).Return(equipments_service.Equipments{
					Id:            1,
			...
			mockUsers: func(m *mock_users_service.MockUsersRepo) {
				m.EXPECT().FindById("410dkjfboi").Return(users_service.UsersResponse{
					Id:            "hello world",
		...
		},
		{
			...
			inputRental: rentals_service.Rentals{
				...
				UserId:      "410dkjfboi"},
			mockRentals: func(m *mock_rentals_service.MockRentalsRepo) {
				m.EXPECT().Create(gomock.Any()).Return(rentals_service.Rentals{
					Id:           1,
					UserId:       "213cgzjhk",
					EquipmentId:  1,
					RentalPeriod: "dddd",
			...
			mockRentalHistories: func(m *mock_rental_histories_service.MockRentalHistoriesRepo) {
				m.EXPECT().CreateRentalHistory(gomock.Any()).Return(rental_histories_service.RentalHistories{}, errors.New("There is no data"))
			},
			mockXendit:   func(m *mock_xendit_service.MockXenditRepo) {},
			mockPayments: func(m *mock_payments_service.MockPaymentsRepo) {},
			wantErr:      true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr {
				assert.Equal(t, rentals_service.RentalsWithInvoiceUrl{}, rentalWithInvoice)
				assert.NotNil(t, err)
			} else {
				assert.Nil(t, err)
			}
